...
[![Build Status](https://travis-ci.org/OpenNMS/osgi-jax-rs-connector.svg?branch=master)](https://travis-ci.org/OpenNMS/osgi-jax-rs-connector) [![Maven Status](https://maven-badges.herokuapp.com/maven-central/org.opennms.eclipsesource.jaxrs/publisher/badge.png)](http://search.maven.org/#search%7Cga%7C1%7Cg%3A%22org.opennms.eclipsesource.jaxrs%22) [![License](http://img.shields.io/badge/license-EPL-blue.svg)](http://www.eclipse.org/legal/epl-v10.html)
...
* `org.opennms.eclipsesource.jaxrs.provider.moxy` - Allows the de/serialization using [EclipseLink MOXy](https://www.eclipse.org/eclipselink/#moxy).  
* `org.opennms.eclipsesource.jaxrs.provider.gson` - Allows the de/serialization using [Gson](https://code.google.com/p/google-gson/).  
* `org.opennms.eclipsesource.jaxrs.provider.security` - Provides an OSGi friendly integration of Jersey's/JAX-RS security features. [Read the wiki for more information](https://github.com/hstaudacher/osgi-jax-rs-connector/wiki/security).
* `org.opennms.eclipsesource.jaxrs.provider.sse` - Provides an integration of Jersey's [SSE feature](https://jersey.java.net/documentation/latest/sse.html) *(requires javax.servlet 3.x)*.
* `org.opennms.eclipsesource.jaxrs.provider.multipart` - Provides an integration of Jersey's [Multipart feature](https://jersey.java.net/documentation/latest/media.html#multipart).
* `org.opennms.eclipsesource.jaxrs.provider.swagger` - Provides an integration of [Swagger](http://swagger.io/). See the [swagger wiki page](https://github.com/hstaudacher/osgi-jax-rs-connector/wiki/Swagger-Integration) for more details.
...
### Maven
All artifacts of this projects are available in Maven Central with the group id [`org.opennms.eclipsesource.jaxrs`](http://search.maven.org/#search%7Cga%7C1%7Cg%3A%22org.opennms.eclipsesource.jaxrs%22).
...
1. Add the org.opennms.eclipsesource.jaxrs.connector.feature (OSGi JAX-RS Connector) to your target using the url mentioned in the Installation/p2 section.
2. Add the `org.opennms.eclipsesource.jaxrs.publisher` and its dependencies bundles to your OSGi instance.
...
### Publisher Examples
Some examples exist for the publisher. They are located in the bundles `org.opennms.eclipsesource.jaxrs.connector.example` and `org.opennms.eclipsesource.jaxrs.connector.example.ds`. As the names say one uses Declarative services and the other don't.
...
As a bonus the `org.opennms.eclipsesource.jaxrs.connector.example.ds` contains an example how to use the `@Provider` together with the publisher.

Besides these basic example two example exist that are shwoing how the [security integration](https://github.com/hstaudacher/osgi-jax-rs-connector/tree/master/examples/org.opennms.eclipsesource.jaxrs.security.example) and [Jersey's SSE integration](https://github.com/hstaudacher/osgi-jax-rs-connector/tree/master/examples/org.opennms.eclipsesource.jaxrs.sse.example) works.

### Consumer Examples
The example for the consumer is splitted into two bundles called `org.opennms.eclipsesource.jaxrs.consumer.example` and `org.opennms.eclipsesource.jaxrs.consumer.example.caller`. The example bundle contains the data model and the resource interface while the caller just calls the service. The example fetches data from github and is pretty simple.
...
## Jersey version
With Jersey 2.0 the library was splitted into several modules with a whole bunch of dependencies. To ease the OSGi application development Jersey was rebundled in this project and ships as a single bundle called `org.opennms.eclipsesource.jaxrs.jersey.all`. Currently it includes Jersey 2.22.2 and it's dependencies. Also the Eclipse Source Bundle will be shipped to make the Jersey API more discoverable when using Eclipse. Anyway, it's up to you to use this 'all' bundle or to fetch the single Jersey bundles yourself. The connector just imports packages.
